...
describe('enqueueKeyedTask', () => {
  ...
  it('cleans up tails map after all tasks for a key complete', async () => {
    const tails = new Map<string, Promise<void>>()
    await enqueueKeyedTask({ tails, key: 'x', task: async () => 1 })
    await Promise.resolve()
    expect(tails.has('x')).toBe(false)
  })
})
...
describe('KeyedAsyncQueue', () => {
  ...
  it('serializes tasks for the same key via class interface', async () => {
    const queue = new KeyedAsyncQueue()
    const order: number[] = []
    const gate = deferred<void>()
  ...
  it('runs different keys concurrently', async () => {
    ...
    const t1 = queue.enqueue('a', async () => {
      started.push('a')
      await gate.promise
    })
    const t2 = queue.enqueue('b', async () => {
      started.push('b')
    })
