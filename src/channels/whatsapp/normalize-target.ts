// Adapted using referensi/openclaw/extensions/whatsapp/src/normalize-target.ts
...
/**
 * Normalize a phone number to E.164-like format (digits only, no +).
 * Returns the digits string if valid, or empty string if not.
 */
function normalizeE164(value: string): string {
  ...
  // Return with leading + if it looks like an international number
  return `+${digits}`
}
...
function extractUserJidPhone(jid: string): string | null {
  ...
  if (legacyUserMatch) {
    return legacyUserMatch[1] ?? null
  }
  const lidMatch = jid.match(WHATSAPP_LID_RE)
  if (lidMatch) {
    return lidMatch[1] ?? null
  }
  return null
}
