...
} from '../../plugin-sdk/channel-lifecycle.js';
import { buildTelegramThreadParams, type TelegramThreadSpec, normalizeTelegramReplyToMessageId } from './bot-helpers.js';
...
type TelegramSendMessageParams = any; // Parameters<Bot['api']['sendMessage']>[2];
...
export type SupersededTelegramPreview = {
  messageId: number;
  textSnapshot: string;
  parseMode?: 'HTML';
...
export function createTelegramDraftStream(params: {
  ...
  const sendRenderedMessageWithThreadFallback = async (sendArgs: {
    ...
    const sendParams: any = sendArgs.renderedParseMode
      ? {
          ...replyParams,
    ...
    const usedThreadParams = typeof sendParams?.message_thread_id === 'number';
    try {
      return {
  ...
  const sendOrEditStreamMessage = async (text: string): Promise<boolean> => {
    ...
    try {
      ...
      streamState.stopped = true;
      params.warn?.(`telegram stream preview failed: ${String(err)}`);
      return false;
    }
  };
  ...
  const clear = async () => {
    ...
    if (typeof messageId === 'number' && Number.isFinite(messageId)) {
      try {
        ...
        params.warn?.(`telegram stream preview cleanup failed: ${String(err)}`);
      }
      return;
    }
  };
